...
const Event =  ({reversePitch}) => {

  const [remainingDays, setRemainingDays] = useState(null);
  useEffect(() => {
    if (reversePitch && reversePitch.end_date) {
      const endDate = new Date(reversePitch.end_date);
  ...
  return (
    ...
    <div className='md:w-[45%] flex flex-col justify-center gap-12'>
        <div>
            <h1 className='xl:text-[50px]  text-[40px] font-medium text-[#FF8500] uppercase'>{reversePitch?.ruleHeading}</h1>
            <p className='xl:text-[50px]  text-[40px] font-medium text-[#4C4C4D]'>{reversePitch?.ruleDescription}</p>
        </div>
        ...
        <div className='lg:flex flex-col gap-5 hidden'>
            <ul className='flex gap-7 text-[16px] text-[#1C2126] list-disc  flex-col md:grid grid-cols-4 ml-4'>
                {
                    reversePitch?.ruleList?.slice(0, 3).map((item, index) => (
        ...
        <div className='box-shadow rounded-[23px] flex flex-col justify-center md:p-5'>
            ...
            <div className='flex gap-20 p-5'>
                <div className='flex gap-2' >
                    ...
                    <p className='text-[16px] text-[#000000] font-medium'>{reversePitch?.mode}</p>
                </div>
                <div className='flex gap-2'>
                    <Image src="/hackathone/home123.png" className="w-6 h-6" width={200} height={200} alt=""></Image>
                    <p className='text-[16px] text-[#000000] font-medium'>{reversePitch?.type}</p>
                </div>
            </div>
            <div className='flex gap-10 p-5'>
                <p className='text-[16px] #1C2126 '><span className='text-[#000000] font-medium'>{reversePitch?.prize}</span> in prizes</p>
                <p className='text-[16px] #1C2126 '><span className='text-[#000000] font-medium'>{reversePitch?.participants}</span> {" "} participants</p>
            </div>
            <div className='flex gap-10 p-5'>
            ...
            <div className='flex gap-2 p-5'>
                ...
                <div className='grid grid-cols-3 gap-2' style={{ display: 'flex', flexWrap: 'wrap' }}>
                    {reversePitch?.tags?.map((item, index) => (
            ...
            <div className='flex items-center pb-12 gap-4 px-4'>
                <Image src={reversePitch?.managedByimage} className='w-[100px]' alt=""  width={200} height={200}/>
                <p className='text-[16px] text-[#1C2126] font-normal'>Managed by {reversePitch?.managedByname}</p>
            </div>
        </div>

    </div>
    {active &&
     <ViewFullRules onClose={handleModalClose} item={reversePitch}/>
     }
</div>
  );
};
